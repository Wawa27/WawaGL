...
public class GLSimpleMesh extends GLMesh {
    private String name;
    ...
    public GLSimpleMesh(Mesh mesh) {
        this(mesh, new Entity());
    }

    public GLSimpleMesh(Mesh mesh, Entity entity) {
        super(mesh);
        ...
        if (mesh.getTexturePath() != null) texture = AssetManager.getTexture2D(mesh.getTexturePath());
        material = mesh.getMaterial();

        if (mesh.getName().equals("Light")) {
            shader = Shader.loadShader("shaders/vert/world_vertex_shader_light.glsl", "shaders/frag/light.glsl");
        } else {
            shader = (texture == null) ? Shader.getMaterialShader() : Shader.getTextureShader();
        }

        update();
    }

    public GLSimpleMesh(Mesh mesh, Entity entity, Shader shader) {
        super(mesh);
        this.name = mesh.getName();
        vertexArray = new VertexArrayObject(mesh);
        indexBuffer = new IndexBufferObject(mesh.getIndices());
    ...
    public void update() {
        ...
        if (material != null) shader.setMaterial(material);
