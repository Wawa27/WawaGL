...
public class LightSceneUBO extends BufferObject {
    ...
    public void updatePointLight(int lightOffset) {
        ...
        Vector3f position = new Vector3f(lightScene.getPointLights().get(lightOffset).getPosition());
        position.mul(Camera.ACTIVE.getNormalMatrix());
    ...
    public void updateDirectionalLight(int lightOffset) {
        ...
        Vector3f direction = new Vector3f(lightScene.getDirectionalLights().get(lightOffset).getDirection());
        direction.mul(Camera.ACTIVE.getNormalMatrix());
    ...
    public void updateSpotLight(int lightOffset) {
        ...
        Vector4f position = new Vector4f(lightScene.getSpotLights().get(lightOffset).getPosition(), 1f);
        position.mul(Camera.ACTIVE.getMatrix());
        glBufferSubData(TARGET, offset, new float[]{position.x, position.y, position.z});

        Vector4f direction = new Vector4f(lightScene.getSpotLights().get(lightOffset).getDirection(), 1f);
        direction.mul(Camera.ACTIVE.getMatrix());
